...
public class DiffTest extends TestCase {
	public void testDiff() throws BadAlgorithmException, IOException, BadSignalException {
		...
		StockAlgorithm.Init emaInit = TestHelper.getStockAlgorithmInit("ema", "aapl", stockInit.signalsStorage);
		emaInit.settings.addSubExecutionName("in");
		Ema ema = new Ema(emaInit);

		StockAlgorithm.Init smaInit = TestHelper.getStockAlgorithmInit("sma", "aapl", stockInit.signalsStorage);
		smaInit.settings.addSubExecutionName("in");
		Sma sma = new Sma(smaInit);

		StockAlgorithm.Init diffInit = TestHelper.getStockAlgorithmInit("diff", "aapl", stockInit.signalsStorage);
		...
		assertEquals(0.0,
				stockInit.signalsStorage.getStockSignal("aapl", "diff", 0).getSignal(DoubleSignal.class).value, 0.001);
		assertEquals(0.928176,
				stockInit.signalsStorage.getStockSignal("aapl", "diff", 4).getSignal(DoubleSignal.class).value, 0.001);
	}
}
