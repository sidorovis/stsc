...
public class Main {
	private static class ImportCrawlDataAssembly extends SubAssembly {
		...
		public ImportCrawlDataAssembly(String name) {
			...
			// replace ":nl" with a new line, return the fields "url" and "page"
			// to the stream.
	...
	private static class WordCountSplitAssembly extends SubAssembly {
		...
		private static final long serialVersionUID = 4375370206386541270L;

		public WordCountSplitAssembly(String sourceName, String sinkUrlName, String sinkWordName) {
			// create a new pipe assembly to create the word count across all
			// the pages, and the word count in a single page
			Pipe pipe = new Pipe(sourceName);
			// convert the html to xhtml using the TagSouParser. return only the
			// fields "url" and "xml", discard the rest
			pipe = new Each(pipe, new Fields("page"), new TagSoupParser(new Fields("xml")), new Fields("url", "xml"));
			// apply the given XPath expression to the xml in the "xml" field.
			// this expression extracts the 'body' element.
			...
			// apply the regex to break the document into individual words and
			// stuff each word at a new tuple into the current
	...
	public static void main(String[] args) {
		...
		// import a text file with crawled pages from the local filesystem into
		// a Hadoop distributed filesystem
		// the imported file will be a native Hadoop sequence file with the
		// fields "page" and "url"
		// note this examples stores crawl pages as a tabbed file, with the
		// first field being the "url"
		...
		// a predefined pipe assembly that splits the stream into two named
		// "url pipe" and "word pipe"
		// these pipes could be retrieved via the getTails() method and added to
		// new pipe instances
		...
		// create an assembly to export the Hadoop sequence file to local text
		// files
