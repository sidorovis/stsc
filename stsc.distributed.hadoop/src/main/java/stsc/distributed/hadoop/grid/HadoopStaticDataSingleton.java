...
class HadoopStaticDataSingleton {
	...
	static StockStorage getStockStorage(final String dataFolder, final String filteredDataFolder) throws ClassNotFoundException, IOException, InterruptedException {
	...
	private static void fillFactory(FromToPeriod period, SimulatorSettingsGridFactory settings) throws BadParameterException, BadAlgorithmException {
		...
		settings.addStock(
				"level",
				algoStockName("Level"),
				new AlgorithmSettingsIteratorFactory(period).add(new MpDouble("f", 15.0, 20.0, 1.0)).add(
		...
		factoryPositionSide.add(new MpDouble("ps", 50000.0, 200001.0, 20000.0));
