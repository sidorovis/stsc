...
public class DatabaseSettingsStorage {
	...
	public DatabaseSettingsStorage(final DatabaseSettings databaseSettings) throws IOException, SQLException {
		...
		Validate.isTrue(source.isOpen(), "Connection to database should be open");
	...
	private ConnectionSource getConnectionSource() throws IOException, SQLException {
		return new JdbcConnectionSource(databaseSettings.getJdbcUrl());
	}

	public CreateOrUpdateStatus createOrUpdateCategory(OrmliteYahooDatafeedSettings newCategory) throws SQLException {
